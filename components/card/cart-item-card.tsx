    ...
    <div className="flex gap-4 py-4 border-b items-center border-border last:border-b-0">
      ...
      <div className="flex-1">
        ...
        {item.type === "ownPizza" && item.ownPizzaId && (
          <div className="text-xs flex flex-wrap gap-3 text-muted-foreground mb-3 space-y-1">
            <p>Crust: {item.ownPizzaId.crust.name}</p> |
            <p>Size: {item.ownPizzaId.size.name}</p> |
            {item.ownPizzaId.toppings.length > 0 && (
              <p>
                Toppings:{" "}
                {item.ownPizzaId.toppings
                  .slice(0, 2)
                  .map((t) => t.toppingId.name)
                  .join(", ")}
                {item.ownPizzaId.toppings.length > 2 &&
                  ` and ${item.ownPizzaId.toppings.length - 2} more`}
              </p>
            )}
          </div>
        )}
      ...
      <div className="flex flex-col  justify-between h-full">
        ...
        <p className="text-sm md:text-xl mt-2 font-semibold mb-3 bg-gradient-to-br from-[#FB2C36] to-[#F6339A] bg-clip-text text-transparent">
