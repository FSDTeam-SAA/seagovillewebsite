...
export const DetailsTop = () => {
  ...
  const selectedSizeData = sizeOptions[selectedSize === 'small' ? 0 : selectedSize === 'mid' ? 1 : 2];
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="flex flex-col gap-4">
            <div className="relative bg-white rounded-lg overflow-hidden shadow-lg h-[400px] md:h-[500px] w-full">
              <Image
                src="hot-deals.png"
            ...
            <div className="flex gap-3 overflow-x-auto">
              {[...Array(6)].map((_, i) => (
                <button
                  key={i}
                  className="relative w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 border-2 border-transparent hover:border-red-500 transition-colors"
                >
                  <Image
                    src="/detals.jpg"
                    alt={`Pizza view ${i + 1}`}
          ...
          <div className="flex flex-col gap-6">
            {/* Title & Rating */}
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">Classic Pepperoni</h1>
            ...
            {/* Size Selection */}
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Choose Your Size</h3>
              <div className="grid grid-cols-3 gap-3">
                {sizeOptions.map((option, idx) => (
                  <button
                    key={idx}
                    onClick={() => setSelectedSize(idx === 0 ? 'small' : idx === 1 ? 'mid' : 'large')}
                    className={`p-3 rounded-lg border-2 text-center transition-all ${
                      selectedSize === (idx === 0 ? 'small' : idx === 1 ? 'mid' : 'large')
                  ...
                  >
                    ...
                    <p className="text-red-600 font-bold text-lg mt-2">${option.price}</p>
                  </button>
                ))}
              </div>
            </div>
