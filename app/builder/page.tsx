...
export default function BuilderPage() {
  ...
  const { state, setSize, setCrust, setSauce, setCheese, addTopping, removeTopping } = usePizzaBuilder()
  ...
  const handleAddToCart = () => {
    ...
    const pizzaName = [state.size.name, state.crust.name, state.cheese.name].filter(Boolean).join(" - ")
      ...
      <div className="flex-1 py-8 md:py-12">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-bold">Create Your Own Pizza</h1>
            <p className="text-muted-foreground mt-2">Customize your pizza just the way you like it</p>
          </div>
              ...
              <div className="bg-card rounded-lg border border-border p-6 md:p-8">
                ...
                {currentStep === 0 && (
                  <div>
                    ...
                    <SelectionGrid items={SIZES} selectedId={state.size?.id} onSelect={setSize} />
                  </div>
                )}
                ...
                {currentStep === 1 && (
                  <div>
                    ...
                    <SelectionGrid items={CRUSTS} selectedId={state.crust?.id} onSelect={setCrust} />
                  </div>
                )}
                ...
                {currentStep === 2 && (
                  <div>
                    ...
                    <SelectionGrid items={SAUCES} selectedId={state.sauce?.id} onSelect={setSauce} />
                  </div>
                )}
                ...
                {currentStep === 3 && (
                  <div>
                    ...
                    <SelectionGrid items={CHEESES} selectedId={state.cheese?.id} onSelect={setCheese} />
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="mb-6 p-4 bg-secondary/50 rounded-lg">
                      <p className="text-sm text-muted-foreground">
                        Add up to 10 toppings to your pizza. Toppings are optional.
                      </p>
                    </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {TOPPINGS.filter((t) => t.category === "meat").map((topping) => (
                          <button
                            key={topping.id}
                            onClick={() => {
                              const isSelected = state.toppings.some((t) => t.id === topping.id)
                              if (isSelected) {
                            ...
                            disabled={!state.toppings.some((t) => t.id === topping.id) && state.toppings.length >= 10}
                            className={`
                              ...
                              ${
                                !state.toppings.some((t) => t.id === topping.id) && state.toppings.length >= 10
                          ...
                          >
                            ...
                            <p className="text-xs text-primary">+${topping.price.toFixed(2)}</p>
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-3 mb-8">
                      <h3 className="font-semibold text-lg">Vegetable Toppings</h3>
                      <div className="grid grid-cols-2 gap-3">
                    ...
                    {state.toppings.length > 0 && (
                      <div className="p-4 bg-secondary/50 rounded-lg">
                        <p className="text-sm font-semibold mb-3">Selected Toppings:</p>
                        <div className="flex flex-wrap gap-2">
                          {state.toppings.map((topping) => (
                            ...
                            >
                              {topping.name}
                              <button onClick={() => removeTopping(topping.id)} className="hover:opacity-75">
                                <X className="w-3 h-3" />
                              </button>
                            </div>
                          ))}
                ...
                {currentStep === 5 && (
                  <div>
                    ...
                    <div className="space-y-6">
                      ...
                      <p className="text-muted-foreground text-sm">Ready to add this pizza to your cart?</p>
                    </div>
                  </div>
                )}
              </div>
